...
{
    public partial class Form_DT_Difference : Form
    {
        private DataTable _dt = new DataTable();
        int n, m;
        string variable_name;

        public Form_DT_Difference()
        {
            InitializeComponent();
            this.Text = "数据实用工具-差分";
        }

        private void Form_DT_Difference_Load(object sender, EventArgs e)
        {
        ...
        //生成并保存差分数据
        {
            ...
            for (int i = 0;i < n; i++)
            {
                ...
                string name = "差分(" + d.ToString() + ")(" + _dt.Columns[i].ColumnName + ")";
                if (_dt.Columns.Contains(name)) continue; //对应已经实现过同样的差分的数据，跳过
                ...
                for (int j = d; j < m; j++)
                {
                    if ((double)_dt.Rows[j][i] == Dataset_Manager.Dataset.INF || (double)_dt.Rows[j - d][i] == Dataset_Manager.Dataset.INF)
                        _dt.Rows[j][name] = Dataset_Manager.Dataset.INF;
                    else 
                        _dt.Rows[j][name] = (double)_dt.Rows[j][i] - (double)_dt.Rows[j - d][i];
                }
